...
import { YTDLP, YTDLPEvent } from './ytdlp';
...
const createWindow = (): void => {
  ...
  const ytdlp = new YTDLP(app.getPath('userData'));
  ytdlp.on(YTDLPEvent.DownloadProgress, (progress: string) => {
    mainWindow.webContents.send(IpcEvent.YtdlpDownloadProgress, progress);
  });
  ipcMain
    .removeAllListeners(IpcEvent.IsYtdlpInstalled)
    .handle(IpcEvent.IsYtdlpInstalled, async (event): Promise<boolean> => {
      return await ytdlp.isInstalled();
    });
  ipcMain
    .removeAllListeners(IpcEvent.DownloadYtdlp)
    .handle(IpcEvent.DownloadYtdlp, async (event): Promise<void> => {
  ...
  ipcMain
    ...
    .handle(IpcEvent.StartDownload, async (_, url: string): Promise<string> => {
      return new Promise(async (resolve, reject) => {
        ...
        downloader
          .start(ytdlp.getFilePath(), url, filePath)
          .then(() => {
  ...
  // const ytdlp = new YTDLP(app.getPath('userData'));
  // ytdlp.on(YTDLPEvent.DownloadProgress, (info: string) => {
  //   console.log('download progress:', info);
  // });
  // ytdlp.isInstalled()
  //   .then(ytdlpInstalled => {
  //     console.log('is installed?', ytdlpInstalled);
  //     console.log('filepath:', ytdlp.filePath);
  //     if (!ytdlpInstalled) {
  //       ytdlp.download()
  //         .then(() => {
